...
class ConvLSTMModel(Sequential):
    def __init__(self, channels, pixels_x, pixels_y, mask_value=-1):
        ...
        self.add(ConvLSTM2D(filters=32, kernel_size=(3, 3), activation='tanh', padding='same',
        ...
        self.add(ConvLSTM2D(filters=128, kernel_size=(3, 3), recurrent_activation='hard_sigmoid', padding='same'
                            , activation='tanh', return_sequences=True, name='convlstm2d_4'))
        self.add(BatchNormalization())
        self.add(MaxPooling3D(pool_size=(1, 2, 2), padding='valid', data_format='channels_last'))
        self.add(LeakyReLU())

        self.add(ConvLSTM2D(filters=256, kernel_size=(3, 3), recurrent_activation='hard_sigmoid', padding='same'
